...
skyrm_temp = SharedArray{Float64,4}(length(Temperature),length(J_space),4,nprocs()-1)
skyrm_err_temp = SharedArray{Float64,4}(length(Temperature),length(J_space),4,nprocs()-1)

mag_temp = SharedArray{Float64,4}(length(Temperature),length(J_space),4,nprocs()-1)
mag_err_temp = SharedArray{Float64,4}(length(Temperature),length(J_space),4,nprocs()-1)
...
for i in 1:2000
    if(mean(proc_complete) == 1)
        ...
        @save "data"*string(N)*"pipi.jld2" skyrm_temp skyrm_err_temp mag_temp mag_err_temp Temperature N J_space
	break
    end
    println(proc_complete)
    sleep(250)
